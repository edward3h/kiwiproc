...
* *Java → SQL*: on PostgreSQL and H2, a `String` parameter is bound using `setObject` with `Types.OTHER` rather than `setString`, because both databases reject (PostgreSQL) or silently mishandle (H2 double-encodes it as a JSON string value) a plain `VARCHAR` bind against a `json`/`jsonb` column. On MySQL and SQLite, the parameter binds as an ordinary `String`; both databases' JDBC drivers report no usable parameter metadata at all for most parameters, JSON included, so there is no PostgreSQL/H2-style special case to trigger — a plain bind is what those databases already expect.
