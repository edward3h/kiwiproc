...
`json`/`jsonb` columns and parameters (PostgreSQL, H2) and columns declared with a `JSON` type name (MySQL, SQLite) map to `String`: the raw JSON text, read and written as-is.
Since Kiwiproc maps JSON as `String`, use a JSON library to serialise and deserialise objects.

* *SQL → Java*: the column's raw text is returned via `getString`, unmodified. Some databases normalise or reformat the stored text on write (for example, PostgreSQL and MySQL both insert a space after `:` in the canonical form they store); what you read back may not be byte-identical to what you inserted.
