...
public abstract class KiwiProcConfigTask extends DefaultTask {
    ...
    public void run() {
        ...
        } else if (nodc.isEmpty() && extension != null && extension.getLiquibaseChangelog().isPresent()) {
        ...
        var dataSources = nodc.stream()
                .map(this::toDataSourceConfig)
                .collect(Collectors.toMap(DataSourceConfig::named, x -> x));
        var processorConfig = new ProcessorConfig(dataSources, getDependencyInjectionStyle().get(), getDebug().get());
    ...
    private void toProperties(Properties properties, ProcessorConfig processorConfig) {
        processorConfig.dataSources().values().forEach(
                dataSourceConfig -> properties.setProperty(
                        "datasources.%s.url".formatted(dataSourceConfig.named()),
    ...
    private DataSourceConfig toDataSourceConfig(KiwiProcDataSource kiwiProcDataSource) {
        ...
        return new DataSourceConfig(kiwiProcDataSource.getName(),
                "jdbc:postgresql://localhost:%d/%s?user=%s".formatted(connectionInfo.getPort(), connectionInfo.getDbName(), connectionInfo.getUser()),
                connectionInfo.getDbName(), connectionInfo.getUser(), null, null);
    }
    ...
    private DataSourceConfig externalDataSourceConfig(KiwiProcDataSource kiwiProcDataSource) {
        if (kiwiProcDataSource.getLiquibaseChangelog().isPresent()) {
            ...
            liquibaseUpdate(kiwiProcDataSource.getLiquibaseChangelog().getAsFile().get(), pgSimpleDataSource);
        }

        return new DataSourceConfig(kiwiProcDataSource.getName(), kiwiProcDataSource.getJdbcUrl().get(), kiwiProcDataSource.getDatabase().get(), kiwiProcDataSource.getUsername().get(), kiwiProcDataSource.getPassword().getOrNull(), null);
    }
    ...
    private void liquibaseUpdate(File file, DataSource dataSource) {
        try (var connection = dataSource.getConnection()) {
            ...
            var liquibase = new Liquibase(file.getName(), new DirectoryResourceAccessor(file.getParentFile()), liquibaseConnection);
            liquibase.update();
